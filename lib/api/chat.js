...
function callGeminiAPI(apiKey, enhancedMessages, model) {
  ...
  return fetch(url, {
    ...
    .then((response) => {
      if (!response.ok) {
        console.error(
          `Error [${response.status}] calling Gemini API: ${response.statusText}`
        );
...
function callOpenAIAPI(apiKey, enhancedMessages, model) {
  ...
  return fetch(url, {
    ...
    .then((response) => {
      if (!response.ok) {
        console.error(
          `Error [${response.status}] calling OpenAI API: ${response.statusText}`
        );
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      return response.json();
...
function callClaudeAPI(apiKey, enhancedMessages, model) {
  ...
  return fetch(url, {
    ...
    .then((response) => {
      if (!response.ok) {
        console.error(
          `Error [${response.status}] calling Claude API: ${response.statusText}`
        );
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      return response.json();
