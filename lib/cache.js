...
export function getCachedChatResponses() {
...
export function setCachedChatResponses(messages, totalTokenCount) {
  const postKey = getCurrentPostKey();
  if (!postKey) return;
  try {
    const key = CACHE_KEYS.CHAT_RESPONSES + postKey;
...
export function clearCachedChatResponses() {
  const postKey = getCurrentPostKey();
  if (!postKey) return;
  try {
    const key = CACHE_KEYS.CHAT_RESPONSES + postKey;
...
export function setCachedChatHistory(history) {
  const postKey = getCurrentPostKey();
  if (!postKey) return;
  try {
    const key = CACHE_KEYS.CHAT_HISTORY + postKey;
...
export function addChatToHistory(chatData) {
