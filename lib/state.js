...
import { callChatAPI, fetchOpenRouterModels } from "./api/chat.js";
...
export class ChatStateManager {
  ...
  getTitle = () => {
    return this.component.props.field.get("label", "Assistant");
  };
  ...
  setProvider = (id) => {
    ...
    let loadModels = Promise.resolve.bind(Promise);
    if (selectedProvider === "openrouter") {
      loadModels = this.loadOpenRouterModels;
    }
    loadModels().then(() => {
      const selectedLLM = this.getModelOrDefault(selectedProvider).id;
      ...
      this.setState(
        ...
        () => {
          setCachedSelectedModel({ selectedProvider, selectedLLM });
        }
      );
    });
  };
  ...
  scrollToBottom = () => {
    ...
    this.setState({ focusedMessageIndex: messages.length });
  };
  ...
  scrollToMessage = (index) => {
    ...
    this.setState({ focusedMessageIndex: index });
  };
  ...
  clearAllHistory = () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to erase ALL chat history (${chatHistory.length} chats) across all posts? This action cannot be undone.`
      )
    ) {
      return;
    }
  ...
  sendCurrentMessage = () => {
    ...
    this.setState(
      { currentMessage: "", isLoading: true, error: null },
      this.scrollToBottom
    );

    this.loadSelectedContent().then(({ content: postContent, attachments }) => {
      ...
      callChatAPI(
        ...
        .then(({ assistantMessage, totalTokenCount }) => {
          ...
          setCachedChatResponses(newMessages, tokensUsed + totalTokenCount);
          this.setState(
            {
              messages: newMessages,
              isLoading: false,
              totalTokenCount: tokensUsed + totalTokenCount,
              selectedPosts: [],
              selectedCodeFiles: [],
  ...
  setApiKey = () => {
    const { apiKeyInput, selectedProvider } = this.getState();
    ...
    setCachedApiKey(selectedProvider, apiKeyInput);
  };
  ...
  deleteChat = (chatData) => {
    if (
      !window.confirm(
        ...
        } messages from ${new Date(chatData.timestamp).toLocaleDateString()}?`
      )
    ) {
      return;
    }
    deleteChatFromHistory(chatData);
    this.loadChatHistory();
  };
    ...
    return new Promise((resolve) => {
      ...
      Object.keys(WIDGET_SETTINGS_CONFIG).forEach((key) => {
        if (settings[key] === undefined || settings[key] === "") {
          if (this.component.props.field.get(key, undefined) !== undefined) {
            settings[key] = this.component.props.field.get(key, settings[key]);
            updated = true;
          } else if (WIDGET_SETTINGS_CONFIG[key].defaultValue !== undefined) {
      ...
      if (updated) {
        setCachedWidgetSettings(settings);
      }
