...
export function callChatAPI(
  ...
  if (selectedProvider === "google") {
    return callGeminiAPI(apiKey, enhancedMessages, model, widgetSettings);
  } else if (selectedProvider === "openai") {
    return callOpenAIAPI(apiKey, enhancedMessages, model, widgetSettings);
  } else if (selectedProvider === "anthropic") {
    return callClaudeAPI(apiKey, enhancedMessages, model, widgetSettings);
  } else if (selectedProvider === "openrouter") {
    return callOpenRouterAPI(apiKey, enhancedMessages, model, widgetSettings);
  } else {
    return Promise.reject(new Error("Unsupported LLM selected"));
  }
}

function callGeminiAPI(apiKey, enhancedMessages, model, widgetSettings) {
  ...
  const requestBody = {
    system_instruction: { parts: [{ text: widgetSettings.systemPrompt }] },
    contents: contents,
    generation_config: {
      temperature: widgetSettings.temperature,
      max_output_tokens: widgetSettings.maxTokens,
    },
  };
...
function callOpenAIAPI(apiKey, enhancedMessages, model, widgetSettings) {
  ...
  const messagesWithSystemPrompt = [
    { role: "system", content: widgetSettings.systemPrompt },
    ...messages,
  ];
  ...
  const requestBody = {
    ...
    max_tokens: widgetSettings.maxTokens,
    temperature: widgetSettings.temperature,
  };
...
function callClaudeAPI(apiKey, enhancedMessages, model, widgetSettings) {
  const url = model.apiBaseUrl;
  ...
  const requestBody = {
    ...
    max_tokens: widgetSettings.maxTokens,
    temperature: widgetSettings.temperature,
    system: widgetSettings.systemPrompt,
    messages: messages,
  };
...
function callOpenRouterAPI(apiKey, enhancedMessages, model, widgetSettings) {
