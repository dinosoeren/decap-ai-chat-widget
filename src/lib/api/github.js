...
export function fetchCodeFileContent(fileUrl) {
  ...
  if (fileUrl.includes(rawGithubBaseUrl) && headers["Authorization"]) {
    ...
    return fetch(apiUrl, { headers })
      ...
      .catch((error) => {
        console.error("Error fetching code file content via API:", error);
        // Attempt fallback to public URL
  ...
  return fetch(fileUrl)
    ...
    .catch((error) => {
      console.error("Error fetching code file content directly:", error);
      return "";
    });
}

export function fetchPostContent(postUrl, settings) {
  ...
  if (source === "github" && headers["Authorization"]) {
    const apiUrl = getPrivateGithubApiUrl(postUrl);
    fetchPromise = fetch(apiUrl, { headers })
      ...
      .catch((error) => {
        console.error("Error fetching post content via API:", error);
        // Attempt fallback to public URL
        return fetch(postUrl, { headers }).then((res) => {
          if (!res.ok) {
    ...
    const fetchHeaders = source === "github" ? headers : {};
    fetchPromise = fetch(postUrl, { headers: fetchHeaders }).then(
      (response) => {
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
  ...
  return fetchPromise.then((content) => {
    ...
    if (source === "github") {
      ...
      processedContent = extractHtmlContent(content, settings.contentSelector);
    }
...
function extractHtmlContent(html, contentSelector) {
